...
const DEFAULT_MAX_REPORT_DATE_RANGE_DAYS: i64 = 31;
...
const MAX_ALLOWED_REQUEST_BODY_BYTES: u64 = 64 * 1024 * 1024;
...
struct Cli {
    ...
    #[arg(
        long,
        default_value_t = DEFAULT_MAX_PAGE_SIZE,
        env = "AWATCH_PORTAL_MAX_PAGE_SIZE"
    )]
    max_page_size: u32,
    ...
    #[arg(
        ...
        env = "AWATCH_PORTAL_DEFAULT_PAGE_SIZE"
    )]
    default_page_size: u32,

    #[arg(
        long,
        default_value_t = DEFAULT_MAX_REPORT_DATE_RANGE_DAYS,
        env = "AWATCH_PORTAL_MAX_REPORT_DATE_RANGE_DAYS"
    )]
    max_report_date_range_days: i64,

    #[arg(
        long,
        default_value_t = DEFAULT_REQUEST_TIMEOUT_SECONDS,
        env = "AWATCH_PORTAL_REQUEST_TIMEOUT_SECONDS"
    )]
    ...
    #[arg(
        ...
        env = "AWATCH_PORTAL_MAX_REQUEST_BODY_BYTES"
    )]
    max_request_body_bytes: u64,

    #[arg(
        long,
        default_value_t = DEFAULT_SLOW_REQUEST_LOG_MS,
        env = "AWATCH_PORTAL_SLOW_REQUEST_LOG_MS"
    )]
    slow_request_log_ms: u64,

    #[arg(long, default_value = "local", env = "AWATCH_PORTAL_ENVIRONMENT")]
    environment: String,

    #[arg(
        long,
        default_value = "executive,workforce,security,forensics,admin",
...
fn validate_portal_config(args: &Cli) -> Result<()> {
    let (host, port) = args
        .bind
        .rsplit_once(':')
...
fn handle_request(request: Request, args: &Cli, snapshot_cache: &SnapshotCache) -> Result<()> {
    ...
    if is_limited_api_route(&path) {
        if let Err(err) = validate_api_query_limits(&url, args) {
            return respond_json_status(request, err.status, &err.payload);
        }
    }
    ...
    match path.as_str() {
        ...
        "/healthz" | "/api/healthz" => respond_json(request, &build_healthz()),
        "/readyz" | "/api/readyz" => {
            let readiness = build_readyz(args);
            let status = if readiness.get("status").and_then(Value::as_str) == Some("ready") {
                StatusCode(200)
            } else {
                StatusCode(503)
            };
            respond_json_status(request, status, &readiness)
        }
        "/version" | "/api/version" => respond_json(request, &build_version(args)),
        "/metrics" | "/api/metrics" => respond_text(
            request,
            StatusCode(200),
            &render_prometheus_metrics(args),
        ...
        "/api/workforce/kpi/explain" => {
            if !role.can_access("workforce")
                && !role.can_access("security")
                && !role.can_access("forensics")
                && !role.can_access("admin")
            {
                return respond_forbidden(request, role, "workforce");
            }
            let snapshot = cached_snapshot(args, snapshot_cache);
            let policy_explain =
                build_workforce_policy_explain(&snapshot, &args.workforce_policy_path, anonymize);
            let query = KpiExplainQuery::from_url(&url);
            respond_json(
                request,
...
fn handle_evidence_only_request(request: Request, args: &Cli) -> Result<()> {
    ...
    if path == "/healthz" || path == "/api/healthz" {
        return respond_json(request, &build_healthz());
    }
    if path == "/readyz" || path == "/api/readyz" {
        ...
        return respond_json_status(request, status, &readiness);
    }
    if path == "/version" || path == "/api/version" {
        return respond_json(request, &build_version(args));
    }
...
fn build_readyz(args: &Cli) -> Value {
...
fn build_version(args: &Cli) -> Value {
    json!({
...
fn is_limited_api_route(path: &str) -> bool {
    matches!(
        path,
        "/api/reports"
            | "/api/executive"
...
fn validate_api_query_limits(url: &str, args: &Cli) -> std::result::Result<(), ApiLimitError> {
...
fn render_prometheus_metrics(args: &Cli) -> String {
...
fn build_workforce_kpi_explain(
    snapshot: &Snapshot,
...
fn render_report_markdown(
    ...
    append_workforce_kpi_explain_markdown(&mut text, context.workforce_kpi_explain);
...
fn append_workforce_kpi_explain_markdown(text: &mut String, explain: &Value) {
    text.push_str("## Объяснение индекса активности\n\n");
    text.push_str(&format!(
        "- KPI score: {}%\n",
        explain
            .get("kpi_score")
    ...
    let coverage = explain.get("coverage").unwrap_or(&Value::Null);
    text.push_str(&format!(
        "- Coverage: agent={}%, freshness={}\n",
        coverage
            .get("agent_coverage_percent")
            .and_then(Value::as_u64)
            .unwrap_or(0),
        coverage
            .get("data_freshness")
        ...
        .take(12)
    {
        text.push_str(&format!(
            ...
            item.get("impact").and_then(Value::as_str).unwrap_or("0"),
    ...
    append_string_list_markdown(
        text,
        "### Предупреждения по KPI",
        explain.get("warnings").and_then(Value::as_array),
        "предупреждений нет",
    );
    append_string_list_markdown(
        text,
        "### Рекомендации по KPI",
        explain.get("recommendations").and_then(Value::as_array),
        "рекомендаций нет",
    );
    text.push('\n');
}

fn append_string_list_markdown(
    text: &mut String,
    title: &str,
...
fn read_limited_body(request: &mut Request, limit: u64) -> Result<String> {
    ...
    if body.len() as u64 > limit {
        return Err(anyhow!("request payload exceeds configured limit"));
    }
    Ok(body)
}
...
fn respond_payload_too_large(request: Request) -> Result<()> {
    respond_json_status(
        ...
        &json!({
            ...
            "message": "request body exceeds configured payload limit"
        }),
    )
}
...
fn handle_evidence_upload(mut request: Request, args: &Cli) -> Result<()> {
    ...
    let body_limit = args
        ...
        .min(32 * 1024 * 1024)
        .min(args.max_request_body_bytes);
    let body = match read_limited_body(&mut request, body_limit) {
        Ok(body) => body,
        Err(err) if is_payload_too_large(&err) => return respond_payload_too_large(request),
        Err(err) => return Err(err),
    };
...
fn http_request_metadata(request: &Request) -> HttpRequestMetadata {
    ...
    let request_id = request_header(request, "X-Request-Id")
        .or_else(|| request_header(request, "X-Request-ID"))
...
fn record_http_metric(metadata: &HttpRequestMetadata, status: StatusCode) {
...
mod tests {
    ...
    #[test]
    fn production_config_validation_rejects_bad_values() {
    ...
    #[test]
    fn heavy_api_query_limits_reject_page_size_and_report_range() {
        let dir = tempfile::tempdir().unwrap();
        let args = test_cli(dir.path());
    ...
    #[test]
    fn workforce_kpi_explain_is_deterministic_and_role_filtered() {
    ...
    #[test]
    fn workforce_kpi_explain_low_confidence_when_data_missing() {
    ...
    #[test]
    fn workforce_kpi_markdown_section_is_rendered() {
        let snapshot = kpi_snapshot(
            true,
            90,
            7 * 3600,
            vec![json!({"application": "ERP", "proved_work_seconds": 7 * 3600})],
        );
        let explain = build_workforce_kpi_explain(
            &snapshot,
            &json!({"configured": true, "index": 88, "matched_applications": 1}),
            PortalRole::Executive,
            &KpiExplainQuery::default(),
            false,
        );
        let mut markdown = String::new();
        append_workforce_kpi_explain_markdown(&mut markdown, &explain);
        assert!(markdown.contains("## Объяснение индекса активности"));
        assert!(markdown.contains("KPI score: 88%"));
